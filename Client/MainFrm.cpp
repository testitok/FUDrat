...
//	m_wndToolBar.EnableDocking(CBRS_ALIGN_ANY); //允许工具栏在工具栏区域中移动
	...
	// 创建托盘图标
	if (!m_TrayIcon.Create(_T("Snips <8.0> 远程管理系统"), // 提示文本
		this,                       // 父窗口
...
//	m_paneManager.SetTheme(xtpPaneThemeVisualStudio2010); // 设置主题
	...
	CXTPDockingPane* pwndPaneLog = CreatePane(300, 160, RUNTIME_CLASS(CLogView), _T("  日志  "), xtpPaneDockBottom);//1
...
//	m_paneManager.AttachPane(pwndPaneplay,pwndPaneLog);
	...
	pwndPaneLog->Select();//1
	pwndPaneLog->SetOptions(xtpThemeVisualStudio2010);//1



//	pwndPaneplay->SetOptions(xtpThemeVisualStudio2010);

	m_paneManager.ShowPane(pwndPaneLog, 1);//1
	// 初始化插件内存
...
// 				return TRUE;
    ...
    SYSTEMTIME sys;
    GetLocalTime(&sys);
	strTime.Format("测试     ",sys.wYear,sys.wMonth,sys.wDay,sys.wDayOfWeek);
	cs.lpszName = strTime;    //改名称
	return TRUE;
...
//	g_pLogView->InsertLogItem( str, 0, 0 );
	...
	// 开启IPCP服务器
 	if (m_iocpServer->Initialize(NotifyProc, nMaxConnections, nPort))
 	{
		...
		strLogText.Format( "监听端口: [ %d ]  端口正确，可以使用", nPort );
