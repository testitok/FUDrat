...
BOOL CScreenSpyDlgs::OnInitDialog() 
{
	...
	//	pSysMenu->AppendMenu(MF_STRING, IDM_DEEP_32, "32位真彩(&G)");
		...
		pSysMenu->CheckMenuRadioItem(IDM_ALGORITHM_SCAN, IDM_DEEP_32, IDM_ALGORITHM_SCAN, MF_BYCOMMAND);
...
void CScreenSpyDlgs::OnPaint() 
{
	...
	StretchBlt(m_hDC, 0, 0, rect.Width(),rect.Height(), m_hMemDC,0,0, m_lpbmi->bmiHeader.biWidth, m_lpbmi->bmiHeader.biHeight, SRCCOPY);
     //BitBlt(m_hDC,0,0,m_lpbmi->bmiHeader.biWidth,m_lpbmi->bmiHeader.biHeight,m_hMemDC,rect.Width(),rect.Height(),SRCCOPY);
	// (BYTE)-1 = 255;
	// Draw the cursor
	if (m_bIsTraceCursor)
		DrawIconEx(
