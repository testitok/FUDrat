...
CClientView::CClientView()
{
	...
	if (((CClientApp *)AfxGetApp())->m_bIsQQwryExist)
	{
		...
		m_gQQwry->SetPath("Plugins//QQWry.Dat");
	}
	else
		m_gQQwry = NULL;
...
//	m_wndTabControl.SetColor(xtpTabColorVisualStudio2010);//分组颜色
...
LRESULT CClientView::OnAddFindGroup(WPARAM wParam, LPARAM lParam)
{
	...
	{	
		...
		for ( int i = 0; i < nTabs; i++)
		{
			...
			{
				lstrcpy( LoginInfo->UpGroup, "Fatal" );//默认分组
			}
...
void CClientView::OnMenuitemSelectNum() 
{
	// TODO: Add your command handler code here
	CInputDialog dlg;
